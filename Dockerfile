...
RUN echo "" && \
    ...
    yum install -y jq vim screen which hostname passwd tmux nano wget git bash-completion openssl shellinabox wget git telnet traceroute iputils httpd-tools util-linux expect --enablerepo=epel && \
    yum clean all && \
    cd /tmp/ && \
    echo "\n=== Installing oc ===" && \
